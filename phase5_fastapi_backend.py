...
def build_features(team1, team2, year):
    elo1 = get_elo(team1); elo2 = get_elo(team2)
    elo_wr1 = elo_win_prob(team1, team2)
    mapping = {
        "elo_diff":       round(elo1 - elo2, 2),
        "alltime_diff":   round(elo_wr1 - (1-elo_wr1), 4),
        "h2h_wr_t1":      0.5,
        "h2h_n_matches":  0,
        "form5_diff":     0.0,
        "form10_diff":    0.0,
    ...
    feat_cols = store.get("features", list(mapping.keys()))
...
# Write files with explicit utf-8 encoding (fixes Windows error)
with open(os.path.join(BACKEND_DIR, "main.py"),
          "w", encoding="utf-8") as f:
    f.write(MAIN_PY)

with open(os.path.join(BACKEND_DIR, "__init__.py"),
          "w", encoding="utf-8") as f:
    f.write("")

print("=" * 62)
print("  PHASE 5 COMPLETE — backend/main.py written")
print("=" * 62)
print()
print("  NOW RUN:")
